...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const addTagError = document.getElementById('add-tag-error');
    const tagsListBody = document.querySelector('#tags-list-table tbody');
    const tagsListContainer = document.getElementById('tags-list-container');
    const tagsTable = document.getElementById('tags-list-table');
    const tagsLoadingMsg = tagsListContainer ? tagsListContainer.querySelector('p') : null;
    ...
    initializePageSelect();       // 頁面選擇器 (圖表)
    setTimeout(() => { displayTrafficChart('daily'); }, 300); // 初始圖表
    setupTagFormListeners();      // 設置標籤表單監聽器
...
async function fetchAndDisplayTags() {
    if (!tagsListBody || !tagsListContainer || !tagsTable) {
        console.error("標籤列表元素未找到。");
        if(tagsLoadingMsg) tagsLoadingMsg.textContent = '頁面結構錯誤。';
        return;
    }
    
    if (tagsLoadingMsg) tagsLoadingMsg.style.display = 'block';
    ...
    try {
        ...
        if (tagsLoadingMsg) tagsLoadingMsg.style.display = 'none';
        if (tagsTable) tagsTable.style.display = 'table';
        tagsListBody.innerHTML = ''; // 清空舊列表
        
        if (tags.length === 0) {
            tagsListBody.innerHTML = '<tr><td colspan="3">目前沒有標籤。</td></tr>';
            return;
        }
        ...
        tags.forEach(tag => {
            ...
            row.innerHTML = `
                ...
                <td>${tag.tag_name}</td>
                <td>
                    <button class="action-btn edit-btn" onclick="openEditTagModal(${tag.tag_id}, '${escapeSingleQuotes(tag.tag_name)}')">編輯</button>
                    <button class="action-btn delete-btn" onclick="deleteTag(${tag.tag_id})">刪除</button>
                </td>
            `;
            tagsListBody.appendChild(row);
        });
        
    } catch (error) {
        console.error("獲取標籤列表失敗:", error);
        if (tagsLoadingMsg) tagsLoadingMsg.textContent = '無法載入標籤列表。';
        if (tagsTable) tagsTable.style.display = 'none';
    }
}

// 輔助函數：轉義單引號，用於 onclick 屬性
...
async function handleAddTag(event) {
    event.preventDefault();
    if (!addTagForm || !newTagNameInput || !addTagError) return;
    addTagError.textContent = '';
    const tagName = newTagNameInput.value.trim();
    if (!tagName) {
        addTagError.textContent = '標籤名稱不能為空。';
        return;
    }
    ...
    try {
        ...
        addTagForm.reset(); // 清空輸入框
        await fetchAndDisplayTags(); // 刷新列表
        // *** 重要: 同時刷新商品表單中的標籤選項 ***
        // await fetchAndPopulateProductTagCheckboxes(); // (下一步實作)
    } catch (error) {
        console.error('新增標籤失敗:', error);
        addTagError.textContent = error.message;
    }
}

window.deleteTag = async function(tagId) {
    if (!confirm(`確定要刪除標籤 ID: ${tagId} 嗎？這會自動移除所有商品上使用此標籤的關聯。`)) return;
    ...
    try {
        ...
        alert(`刪除標籤時發生錯誤：${error.message}`);
    }
}

window.openEditTagModal = function(tagId, tagName) {
    if (!editTagModal || !editTagForm || !editTagIdInput || !editTagNameInput || !editTagError) {
        console.error("編輯標籤 Modal 元件缺失。"); return;
    }
    editTagError.textContent = '';
    editTagIdInput.value = tagId;
    editTagNameInput.value = tagName; // 直接使用傳入的名稱
...
async function handleEditTag(event) {
    event.preventDefault();
    if (!editTagForm || !editTagIdInput || !editTagNameInput || !editTagError) return;
    editTagError.textContent = '';
    const tagId = editTagIdInput.value;
    const tagName = editTagNameInput.value.trim();

    if (!tagName) {
        editTagError.textContent = '標籤名稱不能為空。';
        return;
    }
    if (!tagId) {
         editTagError.textContent = '錯誤：找不到標籤 ID。';
         return;
    }
    ...
    try {
        ...
        editTagError.textContent = error.message;
    }
}

function setupTagFormListeners() {
    if (addTagForm) addTagForm.addEventListener('submit', handleAddTag);
    if (editTagForm) editTagForm.addEventListener('submit', handleEditTag);
    // 點擊 Modal 外部關閉編輯標籤 Modal
