...
document.addEventListener('DOMContentLoaded', () => {
    ...
    async function fetchAndDisplayBanners() {
        ...
        try {
            ...
            console.log("Admin Banners Fetched:", banners); // Debug Log
            ...
            banners.forEach(banner => {
                ...
                // *** 在 row.innerHTML 中加入 page_location ***
                // *** 確保所有 banner.xxx 變數都存在 ***
                row.innerHTML = `
                    <td>${banner.id || 'N/A'}</td>
                    ...
                    <td>${getLocationDisplayName(banner.page_location)}</td> <!-- 顯示中文名稱 -->
             ...
             if (loadingMessage) loadingMessage.textContent = `無法載入輪播圖列表: ${error.message}`; // 在頁面上顯示錯誤
             if (bannerTable) bannerTable.style.display = 'none';
        }
    }
    ...
    async function openEditBannerModal(id) {
        ...
        try {
            // 保持 fetch all and find 的方式，因為管理列表通常不大
             const response = await fetch(`/api/admin/banners`);
             ...
             console.log("找到要編輯的 Banner:", banner); // Debug Log
    ...
    // --- Attach Edit/Delete/ShowAddForm Functions ---
    ...
    window.showAddBannerForm = function() {
        ...
        addForm.reset();
        addBannerPreview.style.display = 'none'; addBannerPreview.src = '';
        addBannerDisplayOrder.value = 0;
        // *** 重置新增表單的下拉選單為預設值 'home' ***
         ...
         if (urlInput && previewImg) {
             urlInput.addEventListener('input', () => {
                 ...
                 if (url) {
                     ...
                     previewImg.onerror = () => { previewImg.style.display = 'none'; previewImg.src = ''; };
                 } else {
    ...
    if (editForm) {
        editForm.addEventListener('submit', async (event) => {
            ...
            const displayOrder = displayOrderInput === '' ? 0 : parseInt(displayOrderInput); // 如果為空，預設為0
            ...
            const updatedData = {
                 ...
                 // *** 從下拉選單讀取 page_location ***
                 page_location: editBannerPageLocation.value
            };
            console.log("準備更新 Banner:", bannerId, updatedData); // Debug Log

            if (!updatedData.image_url) { editFormError.textContent = '圖片網址不能為空。'; return; }
            if (!updatedData.page_location) { editFormError.textContent = '請選擇顯示頁面。'; return; } // 基本驗證
    ...
    if (addForm) {
        addForm.addEventListener('submit', async (event) => {
            ...
            console.log("準備新增 Banner:", newBannerData); // Debug Log

             if (!newBannerData.image_url) { addFormError.textContent = '圖片網址不能為空。'; return; }
             if (!newBannerData.page_location) { addFormError.textContent = '請選擇顯示頁面。'; return; } // 基本驗證
