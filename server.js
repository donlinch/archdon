...
// --- ★★★ 新增：Guestbook Public API Routes ★★★ ---

// GET /api/guestbook - Fetch visible messages, replies (with admin identity name), and pagination
app.post("/api/guestbook", async (req, res) => {
    console.log("收到留言 POST：", req.body); // <-- 加這行
    ...
    try {
      ...
      console.error("留言寫入失敗：", err); // 看這裡有沒有錯
...
app.get('/api/guestbook', async (req, res) => {
    ...
    const page = Math.max(1, parseInt(req.query.page, 10) || 1); // 頁碼至少為 1
    const limit = Math.max(1, parseInt(req.query.limit, 10) || 10); // 每頁數量至少為 1，預設 10
    const offset = (page - 1) * limit;

    const client = await pool.connect(); // 使用 client 以便在同一個連接中執行多個查詢

    try {
        // 1. Fetch total count of visible top-level messages for pagination
        ...
        // 2. Fetch visible top-level messages for the current page
        const messagesResult = await client.query(
            `SELECT id, author_name, content, created_at, like_count -- 加入 like_count
        ...
        if (messages.length > 0) {
            // 3. Fetch all visible replies for these messages, joining with admin identities
            const messageIds = messages.map(m => m.id);
            const repliesResult = await client.query(
                `SELECT
                    r.id, r.message_id, r.parent_reply_id,
                    r.author_name, -- 訪客名稱
                    r.content, r.created_at,
                    r.is_admin_reply,
                    r.like_count, -- 加入 like_count
                    ai.name AS admin_identity_name -- 管理員身份名稱
                 FROM guestbook_replies r
                 LEFT JOIN admin_identities ai ON r.admin_identity_id = ai.id -- ★ 左連接身份表
                 WHERE r.is_visible = TRUE AND r.message_id = ANY($1::int[])
                 ORDER BY r.created_at ASC`, // 按時間正序排列回覆
                [messageIds]
            );

            // 4. Group replies by message_id
            repliesResult.rows.forEach(reply => {
                if (!repliesByMessageId[reply.message_id]) {
                    repliesByMessageId[reply.message_id] = [];
                }
                // 如果是管理員回覆但身份名稱為 NULL (可能身份被刪)，給予預設值
                if (reply.is_admin_reply && !reply.admin_identity_name) {
                    reply.admin_identity_name = '管理員(身份已移除)';
                }
        ...
        console.error('獲取留言板資料時出錯:', err.stack || err);
        res.status(500).json({ error: '無法獲取留言板資料' });
    } finally {
        client.release(); // 無論成功或失敗都要釋放連接
    }
});

// POST /api/guestbook - Create a new top-level message
app.post('/api/guestbook', async (req, res) => {
    const { author_name, content } = req.body;

    if (!author_name || !content || author_name.trim() === '' || content.trim() === '') {
        return res.status(400).json({ error: '名稱和內容為必填項。' });
    }
    // 基本長度限制 (可以根據需求調整)
    if (author_name.length > 100 || content.length > 2000) {
        return res.status(400).json({ error: '名稱或內容過長。' });
    }

    try {
        // 這裡 is_visible 預設為 TRUE，如果需要審核，應改為 FALSE
        const result = await pool.query(
            `INSERT INTO guestbook_messages (author_name, content, is_visible, like_count)
             VALUES ($1, $2, TRUE, 0)
             RETURNING id, author_name, content, created_at, like_count, is_visible`, // 回傳必要欄位
            [author_name.trim(), content.trim()]
        );
        res.status(201).json(result.rows[0]); // 回傳新增的留言物件
    } catch (err) {
...
// POST /api/guestbook/replies - Create a new reply (public)
app.post('/api/guestbook/replies', async (req, res) => {
    const { message_id, parent_reply_id, author_name, content } = req.body;

    const msgIdInt = parseInt(message_id, 10);
    // parent_reply_id 可以是 null 或 undefined，所以這樣處理
    const parentIdInt = parent_reply_id ? parseInt(parent_reply_id, 10) : null;

    if (!msgIdInt || !author_name || !content || author_name.trim() === '' || content.trim() === '') {
        return res.status(400).json({ error: '缺少必要資訊或內容為空。' });
    }
    if (isNaN(msgIdInt) || (parentIdInt !== null && isNaN(parentIdInt))) {
    ...
    try {
        // 訪客回覆：is_admin_reply=FALSE, admin_identity_id=NULL, is_visible=TRUE (或 FALSE 如果要審核)
        const result = await pool.query(
            `INSERT INTO guestbook_replies (message_id, parent_reply_id, author_name, content, is_admin_reply, admin_identity_id, is_visible, like_count)
             ...
             RETURNING id, message_id, parent_reply_id, author_name, content, created_at, is_admin_reply, admin_identity_id, like_count, is_visible`, // 回傳必要欄位
            [msgIdInt, parentIdInt, author_name.trim(), content.trim()]
        );
        const newReply = result.rows[0];
        // 即使欄位是 NULL，也明確設定 admin_identity_name，方便前端處理
        newReply.admin_identity_name = null;
        res.status(201).json(newReply); // 回傳新增的回覆物件
    } catch (err) {
        console.error('新增回覆時出錯:', err.stack || err);
        if (err.code === '23503') { // 外鍵約束錯誤
             return res.status(404).json({ error: '找不到欲回覆的留言或父回覆。' });
        }
        res.status(500).json({ error: '無法新增回覆' });
    }
});

// POST /api/guestbook/messages/:id/like - Handle liking a message
app.post('/api/guestbook/messages/:id/like', async (req, res) => {
    const { id } = req.params;
    const msgIdInt = parseInt(id, 10);
    if (isNaN(msgIdInt)) { return res.status(400).json({ error: '無效的留言 ID。' }); }

    try {
        const result = await pool.query(
            // 使用 COALESCE 避免 like_count 是 NULL 的情況 (雖然我們有預設值，但更保險)
            `UPDATE guestbook_messages
             SET like_count = COALESCE(like_count, 0) + 1
             WHERE id = $1 AND is_visible = TRUE -- 只允許對可見留言按讚
        ...
        if (result.rowCount === 0) {
            // 可能是 ID 不存在，或留言已被隱藏
            // 檢查留言是否存在但被隱藏
            const check = await pool.query('SELECT 1 FROM guestbook_messages WHERE id = $1', [msgIdInt]);
            if (check.rowCount > 0) {
                 return res.status(403).json({ error: '無法對已隱藏的留言按讚。' });
            } else {
                 return res.status(404).json({ error: '找不到要按讚的留言。' });
            }
        }
        res.status(200).json({ like_count: result.rows[0].like_count });
    } catch (err) {
        console.error(`處理留言 ID ${id} 按讚時出錯:`, err);
        res.status(500).json({ error: '按讚時發生伺服器內部錯誤' });
    }
});

// POST /api/guestbook/replies/:id/like - Handle liking a reply
app.post('/api/guestbook/replies/:id/like', async (req, res) => {
    const { id } = req.params;
    const replyIdInt = parseInt(id, 10);
    if (isNaN(replyIdInt)) { return res.status(400).json({ error: '無效的回覆 ID。' }); }

    try {
        const result = await pool.query(
            `UPDATE guestbook_replies
             SET like_count = COALESCE(like_count, 0) + 1
             WHERE id = $1 AND is_visible = TRUE -- 只允許對可見回覆按讚
             RETURNING like_count`,
            [replyIdInt]
        );
         if (result.rowCount === 0) {
            const check = await pool.query('SELECT 1 FROM guestbook_replies WHERE id = $1', [replyIdInt]);
            if (check.rowCount > 0) {
                 return res.status(403).json({ error: '無法對已隱藏的回覆按讚。' });
            } else {
                 return res.status(404).json({ error: '找不到要按讚的回覆。' });
            }
        }
        res.status(200).json({ like_count: result.rows[0].like_count });
    } catch (err) {
        console.error(`處理回覆 ID ${id} 按讚時出錯:`, err);
        res.status(500).json({ error: '按讚時發生伺服器內部錯誤' });
    }
});
// --- ★★★ Guestbook Public API Routes END ★★★ ---
