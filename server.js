...
app.post('/api/sales', async (req, res) => {
    ...
    if (!product_name || !quantity || !unit_price) {
        return res.status(400).json({ error: '產品名稱、數量和單價為必填項' });
    }
    
    if (isNaN(quantity) || quantity <= 0) {
        return res.status(400).json({ error: '數量必須是正數' });
    }
    
    if (isNaN(unit_price) || unit_price <= 0) {
        return res.status(400).json({ error: '單價必須是正數' });
    }
    ...
    try {
        ...
        console.error('新增銷售記錄時出錯:', err.stack || err);
        res.status(500).json({ error: '新增銷售記錄時發生內部伺服器錯誤' });
    }
});
...
app.put('/api/sales/:id', async (req, res) => {
    ...
    try {
        ...
        console.error(`更新銷售記錄 ID ${id} 時出錯:`, err.stack || err);
        res.status(500).json({ error: '更新銷售記錄時發生內部伺服器錯誤' });
    }
});
...
app.get('/api/sales/:id', async (req, res) => {
    ...
    try {
        ...
        console.error(`獲取銷售記錄 ID ${id} 時出錯:`, err.stack || err);
        res.status(500).json({ error: '獲取銷售記錄時發生內部伺服器錯誤' });
    }
});
